...
function CreateSchedule(data){
    ...
    //check if the schedule is made for patient
    if (document.URL.toLowerCase().indexOf('patient')>-1){
        scheduleType = 8;
    };
...
function GetDayString(weeks_after,days_after){
    return moment().day(1+((scheduleType+weeks_after)*7)).add(days_after, 'days')._d.getDate()+ "-"+ (moment().day(1+((scheduleType+weeks_after)*7)).add(days_after, 'days')._d.getMonth()+1);
}

function GetDayMonthNames(weeks_after,days_after){
    return moment().day(1+((scheduleType+weeks_after)*7)).add(days_after, 'days')._d.getDate()+ "-"+ monthNames[(moment().day(1+((scheduleType+weeks_after)*7)).add(days_after, 'days')._d.getMonth())];
