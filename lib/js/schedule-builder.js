...
function CreateSchedule(data){
    ...
    for(week = 0; week <9; ++week){
        ...
        for(row = 0; row < 10; ++row){ // 9 row for 9hours shift
...
function CreateAppointment(date_slot, time) {
    console.log('you pressed on ' + date_slot +' at '+ time);
    var dat = {
        time_app:time,
        date_app:date_slot,
        phn_app:phn,
        licence_app:licence_number
    };
    SendRequestAjax("../endpoints-mock-data/success-request.json", "GEt", dat, CloseSchedule, ShowResultModal);
}
